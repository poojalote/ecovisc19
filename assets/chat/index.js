const signalServer = (function () {
			...
			Notification.requestPermission(function (result) {
				...
				if (result === 'granted') {
					...
					/* navigator.serviceWorker.ready.then(function (registration) {
						...
						att1.value = 'https://rmt.docango.com/chat_assets/sound/notification_tune.mp3';
	...
	const remoteUserMessage = (message, category) => {
		...
		if (category == 1) {
			...
			if (user) {
				console.log(user.user_name + " Send Message : " + message.message);
				...
				// $($($('#fav_list_display').find('#person_status_24')[0].parentNode)[0].parentNode).clone().appendTo('#new_list_display')
			...
			if (group) {
				...
				if (!$(`#${group.id}`).is(":visible"))
					updateCounter({sender_id: group.group_id});
			}
	...
	const updateCounter = (message, count = 0) => {
		let updateCounter = 1;
		if (count !== 0) {
			updateCounter = count;
		}
		let updatePanelCounter = 'newMessages';
		if ($($(`#person_status_${message.sender_id}`).children(0)[0]).children('span.count').length == 0) {
			if ($($(`#person_status_${message.sender_id}`).children(0)).length > 0) {
				$($(`#person_status_${message.sender_id}`).children(0)[0]).append('<span class="count"><span class="icon-meta unread-count">' + updateCounter + '</span></span>');
			} else {
				$($(`#person_status_${message.sender_id}`)[0]).append('<span class="count"><span class="icon-meta unread-count">' + updateCounter + '</span></span>');
			}
			...
			if (attr_ul == 'favourite') {
				updatePanelCounter = 'fav_msg_count';
			} else if (attr_ul == 'groups') {
				updatePanelCounter = 'grp_msg_count';
			} else if (attr_ul == 'users') {
				updatePanelCounter = 'user_msg_count';
			}
			if ($($(`#${updatePanelCounter}`).children(0)[0]).children('span.count').length == 0) {
				$($(`#${updatePanelCounter}`).children(0)[0]).append('<span class="count"><span class="icon-meta unread-count">1</span></span>');
			} else {
				let count = parseInt($($($(`#${updatePanelCounter}`).children(0)[0]).children('span.count')).children().html());
				count++;
				$($($(`#${updatePanelCounter}`).children(0)[0]).children('span.count')).children().html(count);
			}
		...
		if ($('#new_list_display').find(`#person_status_${message.sender_id}`).length == 0) {
			if ($($(`#person_status_${message.sender_id}`).children(0)).length > 0) {
				$($($(`#${updatePanelCounter}`).find(`#person_status_${message.sender_id}`)[0].parentNode)[0].parentNode).clone().appendTo('#new_list_display')
			} else {
				$($($(`#${updatePanelCounter}`).find(`#person_status_${message.sender_id}`)[0].parentNode)[0].parentNode).clone().appendTo('#new_list_display')
			}
	...
	const connectUser = (remoteUser) => {
		let isUserExists = roomUsers.findIndex((existingUser) => remoteUser.group_id === existingUser.group_id);
		if (isUserExists === -1) {
			roomUsers.push(remoteUser);
		} else {
			roomUsers[isUserExists] = remoteUser;
		}
	...
	return {
		...
		updateCounter: (message, counter) => updateCounter(message, counter),
	}
})();
