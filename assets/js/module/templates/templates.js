...
function loadTemplateElement(section_id, department_id) {
	...
	app.request(baseURL + "fetch_section_details", formData).then(response => {
...
function fetchSection(department_id) {
	...
	let formData = new FormData();
	formData.set("department_id", department_id)
	app.request(baseURL + "fetch_template_sections", formData).then(response => {
		if (response.status === 200) {
			let sectionTemplate = ``;
			if (response.body.length > 0) {
				sectionTemplate +=`<tr>
