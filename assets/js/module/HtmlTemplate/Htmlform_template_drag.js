...
function querydropdownmultipleselection(id, type, elementId, data = null) {
...
function exceltablebutton(id, type, elementId, data = null) {

	return `<input type="button" class="btn btn-primary" name="${elementId}" id="${elementId}" value="Excel Table Button #${elementId}" disabled="">`;
}

function inlineformtablebutton(id, type, elementId, data = null) {
	return `<input type="button" class="btn btn-primary" name="${elementId}" id="${elementId}" value="Inline Form Table Button #${elementId}" disabled="">`;
}
...
function insertAtCaret(areaId, text, type) {
	...
	// check generate number already exist or not
	if (textNumberArray.length > 0) {
		for (var i = 0; i < textNumberArray.length; i++) {
			if (textNumberArray[i] == textNo) {
				var textNo = Math.floor((Math.random() * 100) + 1);
			}
		}
	}
	...
	switch (type) {
		...
		case "22":
			...
			break;
		case "23":
			items = exceltablebutton(textNo, 23, textName, v_data);
			break;
		case "24":
			items = inlineformtablebutton(textNo, 24, textName, v_data);
			break;
		case "25":
			items = querydropdownmultipleselection(textNo, 25, textName, v_data);
			break;
	}
	// items = row_section(textNo, 21, textName, v_data);
...
function myRemoveChild(element) {
	let x = element.querySelectorAll(".ui-resizable-handle.ui-resizable-s,.ui-resizable-handle.ui-resizable-e,.ui-resizable-handle.ui-resizable-se.ui-icon.ui-icon-gripsmall-diagonal-se");
	for (let i in x) {
		if (x[i].parentNode !== undefined) {
			x[i].parentNode.removeChild(x[i]);
		}

	}
	let y = element.querySelectorAll(".removeDeleteButton");
	for (let i in y) {
		if (y[i].parentNode !== undefined) {
...
function get_addedSectionData(section_id) {
	...
	app.request(baseURL + "htmlform/fetch_template_sectionsHtml", formData).then(response => {
		if (response.status === 200) {
			...
			var doc = parser.parseFromString(userdata.raw_html, 'text/html');

			dynamicDoc = doc;
			// for (let childrenKey in doc.body.children) {
			//
			// 	if (!isNaN(childrenKey)) {
			// 		$('#dropBox').append(doc.body.children[childrenKey]);
			// 	}
			//
			//
			// }
			...
			$("#colorHeaderTextPicker").val(userdata.card_head_text_color);
			if (userdata.theme_view == 2) {
				$('#themeRadios2').prop('checked', true);
			} else if (userdata.theme_view == 3) {
				$('#themeRadios3').prop('checked', true);
			} else {
				$('#themeRadios1').prop('checked', true);
			}
			...
			if (typearr.length > 0) {
				for (var i = 0; i < typearr.length; i++) {
					if (typearr[i] == 3 || typearr[i] == 4 || typearr[i] == 12 || typearr[i] == 13) {
						// console.log(dropDownArray);
						dropDownArray.push({type: typearr[i], text: textarr[i], value: ''});
					}
					if (typearr[i] == 14) {
					...
					if (typearr[i] == 23) {
						design += `<button type="button" class="btn btn-primary m-2" 
						onclick="getExcelTableModal('${userdata.id}','${userdata.department_id}',
						'${textarr[i]}',${typearr[i]},'${userdata.html_section_text}')">` + textarr[i] + `</button>`;
					}
					if (typearr[i] == 24) {
						design += `<button type="button" class="btn btn-primary m-2" 
						onclick="getInlineFormTableModal('${userdata.id}','${userdata.department_id}',
						'${textarr[i]}',${typearr[i]},'${userdata.html_section_text}')">` + textarr[i] + `</button>`;
					}
			...
			if (userdata.opt != null) {
				...
				for (var i = 0; i < myarr.length; i++) {
					...
					if (myop.length > 2) {
						var objIndex = dropDownArray.findIndex((obj => obj.text == myop[1]));
						// console.log(objIndex);
						if (objIndex != -1) {
							dropDownArray[objIndex].value = myop[0];
						} else {
				...
				for (var i = 0; i < reqarr.length; i++) {
					...
					if (reqop.length > 5) {
						requiredFieldArray.push({
							id: reqop[0], type: reqop[1], is_req: reqop[2], default_val: reqop[3],
...
function required_field() {
	...
	for (var i = 0; i < textTypeArray.length; i++) {
		if (textTypeArray[i] != 14 && textTypeArray[i] != 15 && textTypeArray[i] != 16 && textTypeArray[i] != 17 && textTypeArray[i] != 18 && textTypeArray[i] != 19 && textTypeArray[i] != 20 && textTypeArray[i] != 24) {
			var checked = "";
			var value1 = "off";
			var value = "";
			var min_value = "";
...
function addFormDesignOptions() {
	...
	let rawData = document.getElementById("dropBox");

	myRemoveChild(rawData);
	let finalRawData = $("#dropBox").html();
	console.log(finalRawData);
	formData.set("editor", finalRawData);
		...
		for (var i = 0; i < filter_columns.length; i++) {
			...
			for (var j = 0; j < filter.length; j++) {
				...
				if (filterTableColumn.length > 1) {
					...
					if (filterTableColumn[0] == "customfilterValue") {
						$("#filterCustomKeyValue_" + cnt).val(filterTableColumn[1]);
					}
...
function getSelectedFilterSelection(filterColumn, count) {
	...
	for (var j = 0; j < filterColumn.length; j++) {
		...
		if (filterTableColumn.length > 1) {
			if (filterTableColumn[0] == "filterKey") {
				$("#filterKey_" + count + " option[value=" + filterTableColumn[1] + "]").prop("selected", true);
				$("#filterKey_" + count).select2();
			}
			if (filterTableColumn[0] == "filterValue") {
...
function queryDropdownSave(section_id, department_id, id, type) {
	...
	if (myElement != null) {
		...
		if (myEleValue != "") {
			...
			//anyone dependant
			var anyonedep_id = "anydepcount_" + data_id[1];
			var anyonedepValue = document.getElementById(anyonedep_id).value;
			let anyonedepdataArray = [];
			if(anyonedepValue > 0)
			{

				for(var i=1;i<=anyonedepValue;i++)
				{
					var newAnyDepElement = {};
					var anydepcol_id = "anyone_dep_column" + i + "_" + data_id[1] + "";
					var anydepcol_ele = document.getElementById(anydepcol_id);
					if(anydepcol_ele != null)
					{
						var anydepcol_val = document.getElementById(anydepcol_id).value;
						if(anydepcol_val != "")
						{
							var anydeptext_ele = "anyone_dep_query" + i + "_" + data_id[1] + "";
							var anydeptext_val = document.getElementById(anydeptext_ele).value;
							if (anydeptext_val != "") {
								newAnyDepElement['anydep_col'] = anydepcol_val;
								newAnyDepElement['anydep_text'] = anydeptext_val;
								anyonedepdataArray.push(newAnyDepElement);
							}
						}
					}
			...
			newElement1['anydep_op'] = anyonedepdataArray;
		}
...
function getRandomNumber() {
	...
	if (alreadyDone.findIndex(i => i === number) === -1) {
		return number;
	} else {
...
function openParameterModal(section_id, dept_id) {
	...
	app.request("GetQueryParamData", formData).then(response => {
		if (response.status === 200) {
			...
			window.location = baseURL+"html_form_view/" + section_id + "/" + newOBJ;
...
// narendra sir inline form table code //

// function getInlineFormTableModal1(section_id, department_id, id, type, textarray) {
//
//
// 	$("#inlineformtableOpBtn").click();
//
// 	getAllDBTable(1);
// 	$("#haskeyOfInputForm").val(id);
// 	$("#sectionOfInputForm").val(section_id);
// 	$("#departmentOfInputForm").val(department_id);
// }

function getInlineFormTableModal(section_id, department_id, id, type, textarray) {
	...
	$.ajax({
		type: "POST",
		url: baseURL + "getSectionTableData",
		dataType: "json",
		data: {
			id:id,
		...
		success: function (result)
		{
			if (result.status == 200)
			{
				$('#rowStartConfig').css('display','none');
				var data = result.data[0];
				$('#dbTables').val(data.table_name).trigger('change');
				$('#dynamicselectColumns').val(data.fetch_query_columns);
				$('#dynamicWhereCondition').val(data.where_condition);
				$('#dbTables1').val(data.operation_table).trigger('change');
				if(data.operation == 1)
				{
					$('#tableOperation1').attr('checked',true);
				}
				else
				{
					$('#rawQueryOperation1').attr('checked',true);
				}

				let formData = new FormData();
				formData.append('fecth_table_name', data.table_name);
				formData.append('Insert_table_name', data.operation_table);
				formData.append('dynamicselectColumns', data.fetch_query_columns);
				app.request("GetDataColumnsForConfig", formData).then(response => {
					if (response.status == 200) {
						...
						section_colummn_data(id,section_id,department_id)
					}
				});
			}
			else
...
function section_colummn_data(id,section_id,department_id)
{
	$.ajax({
		type: "POST",
		url: baseURL + "getSectionConfigData",
		dataType: "json",
		data: {
			id:id,
			section_id : section_id,
		...
		success: function (result)
		{
			if (result.status == 200)
			{
				// startConfiguration().then(e=>{
				var data = result.data;
				var column_data = [];
				for(i=0;i<data.length;i++)
				{
					var section_data = { header: data[i].header,
						type: data[i].type,
						configuration:data[i].configuration,
						column_name:data[i].column_name,
						operation_column:data[i].operation_column
					};
					column_data.push(section_data);
				}
				$('#dynamicTableConfiguration').rsLiteGrid('setData', column_data);
			}
			else
			{
				app.errorToast(result.body);
...
function getAnyoneDependantColumns(data_id, hashoptionsArray, table_name) {
	var count = $("#anydepcount_" + data_id).val();
	var k = parseInt(count) + 1;
	var hashop='';

	if(hashoptionsArray!="" && hashoptionsArray!=null)
	{
		var hasharray;
		hasharray = hashoptionsArray.split(',');
		hashop=getSelectHashOptions(hasharray);
	}
	// console.log(hashoptionsArray);
	var data = `<div class="mt-2 row" id="div${k}_${data_id}">
				
				<div class="col-md-4">
				<label>Dependant: </label>
				<select name="anyone_dep_column${k}_${data_id}" id="anyone_dep_column${k}_${data_id}" class="form-control">
				` + hashop + `
				</select>
				</div>
				<div class="col-md-7">
				<label>Dependant Query: </label>
				<textarea name="anyone_dep_query${k}_${data_id}" id="anyone_dep_query${k}_${data_id}" class="form-control" 
				row="4" placeholder="Enter dependant query here"></textarea>
				</div>
				<div class="col-md-1">
				<label>Remove dep</label>
				<button type="button" class="btn btn-primary" id="button${k}_${data_id}" onclick="removeAnyoneDepOptions('div${k}_${data_id}','${data_id}',${k})">
				<i class="fa fa-times"></i></button>
				</div>
				<script>$("#anyone_dep_column${k}_${data_id}").select2();
				</script>
			</div>
			
			`;
	$("#anydepcount_" + data_id).val(k);
	$("#txt_anyone_dependant_" + data_id).append(data);
}
function removeAnyoneDepOptions(div_id, data_id, no) {
	var count = $("#anydepcount_" + data_id).val();
	$("#" + div_id).remove();
	for (var i = no; i < count; i++) {
		var j = i + 1;
		var column = "anyone_dep_column" + i + '_' + data_id;
		var value = "anyone_dep_query" + i + '_' + data_id;
		var button = "button" + i + '_' + data_id;
		document.getElementById("anyone_dep_column" + j + '_' + data_id).name = column;
		document.getElementById("anyone_dep_column" + j + '_' + data_id).id = column;

		document.getElementById("anyone_dep_query" + j + '_' + data_id).name = value;
		document.getElementById("anyone_dep_query" + j + '_' + data_id).id = value;
		$("#button" + j + '_' + data_id).attr("onclick", "removeAnyoneDepOptions('div" + i + "_" + data_id + "','" + data_id + "'," + i + ")");
		document.getElementById("button" + j + '_' + data_id).id = button;
		$("#div" + j + '_' + data_id).attr("id", "div" + i + "_" + data_id);
		var data = `<script>$("#anyone_dep_column` + i + `_${data_id}").select2();
				</script>`;
		$("#txt_anyone_dependant_" + data_id).append(data);
	}
	var k = parseInt(count) - 1;
