...
$(document).ready(function () {
	...
	$('#editor').parent().on('keydown', function (event) {
			...
			if (pos) {
				...
				if (r.startOffset == r.endOffset) {
					...
					if (el.classList.contains('highlight')) {
						...
						var elemntIdField=strhtml.trim();
						...
						let elementIndex = textMainInputArray.findIndex(i => i.trim() === elemntIdField);
		...
		event.target.querySelectorAll('p.spanLabel.highlight').forEach(function(el) { el.classList.remove('highlight');})
...
function insertAtCaret(areaId, text,type) {
	...
	var textName1='<span class="spanLabel">'+textName+'</span>';
...
function updateHistory(){
	if($('#history_unabled').is(':checked')){
		insertAtCaret('editor', '#datatable_button_',18);
	}
}
...
function uploadSection(form) {
	var opt_val=$("#QueryStringParameter").val();
	if(opt_val.includes("user_id:int") == false){
		app.errorToast("user_id in query parameters is compulsory.");
	}
...
function getPrimaryColumnCreateField(column,type,buttontype)
{
	...
	textMainInputArray.push(items);
	var textName2='<div class="spanLabel">'+items+'</div>';
	textreturn['textName2']=textName2;
	return textreturn;
...
function getQueryStringPara(){
	...
	app.request("getQueryStringPara").then(response => {
		if (response.status === 200) {
			...
			$("#QueryStringParameter").select2();
			var str=['user_id:int'];
			$("#QueryStringParameter").val(str).trigger("change");
