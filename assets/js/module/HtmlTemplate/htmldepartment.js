...
function loadDepartment(type = 1, companyId = null) {
	...
	app.dataTable("departmentTable", {
		url: baseURL + "getHtmlDepartments",
		data: {
		...
		$('td:eq(4)', nRow).html(`
									<button class="btn btn-primary btn-action mr-1" type="button"                                         
                                          ...
                                          onclick="get_DepartmentDataById(${aData[6]}),modalName()" >
                           				...
                           				<!-- <button class="btn btn-primary btn-action mr-1" type="button"                                         
                                         onclick="setDepartment(${aData[6]},${aData[7]},'${aData[0]}')"
                                         >
                                 			 <i class="fab fa-wpforms"></i>
                            			 </button>-->
                            			 <button class="btn btn-primary btn-action mr-1" type="button"                                         
                                         onclick="setDepartment(${aData[6]},${aData[7]},'${aData[0]}',2)"
                                         >
                                 			 <i class="fab fa-wpforms"></i>
                            			 </button>
                            `);
...
function setDepartment(id,is_admin,department_name,type=null) {
	localStorage.setItem("Department_id", id);
	localStorage.setItem("Department_name", department_name);
	if(type==2)
	{
		window.location.href = baseURL + "Html_template_drag";
	}
	else
	{
		window.location.href = baseURL + "Html_template";
	}
...
function uploadDepartment(form) {
	...
	app.request(baseURL + "saveHtmlDepartment", new FormData(form)).then(result => {
...
function ChangeDepartmentStatus(depId, status) {
	...
	app.request(baseURL + "ChangeHtmlDepartmentStatus", formData).then(success => {
...
function getCompanyDepartment(companyId) {
	...
	$.ajax({
		type: "POST",
		url: base_url + 'HtmlDepartmentController/selectCompanyDepartment',
		dataType: "json",
...
function get_DepartmentDataById(depId) {
	$.LoadingOverlay("show");
	serverRequest(baseURL + "getHtmlDepartmentDataById", {depId: depId}).then(response => {
...
function deleteDeaprtment(depId) {
	$.LoadingOverlay("show");
	serverRequest("HtmlDepartmentController/deleteDeaprtment", {depId: depId}).then(response => {
		$.LoadingOverlay("hide");
		if (response.status === 200) {
...
function getUsersTableData(type = 1, companyId = null) {
	...
	serverRequest("HtmlDepartmentController/getUsersTableData", {type: type, companyId: companyId}).then(response => {
...
$("#uploadUsers").validate({
	...
	submitHandler: function (form) {
		...
		$.ajax({
			type: "POST",
			url: base_url + 'HtmlDepartmentController/uploadUsers',
			dataType: "json",
			data: Form_data,
			contentType: false,
			processData: false,
