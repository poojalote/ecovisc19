...
function prescriptionMedicineTable(name) {
	app.dataTable('prescriptionMedicineTable', {url: baseURL + "getPrescriptionMedicine", data: {name: name}},
		[
			{
				data: 4,
				render: (d, t, r, m) => {
...
function doesGiven(p_id, did, i, date) {
	new Promise((resolve, reject) => {
		$.ajax({
			type: "post",
			url: baseURL + 'doesGivent',
			data: {p_id: p_id, ite_count: i, does_id: did, ite_date: date},
			dataType: "json",
			success: function (result) {
				resolve(result);
			},
