...
<a class="btn btn-link"  href="${baseURL + 'get_patient_data/' + r[5]}" target="_blank" ><i class="fa fa-download"></i></a>
${del_btn}
	...
	else
	{
		...
		else
		{
			if(parseInt(mode)==1){
				...
				// window.location.href = baseURL + "html_navigation/18/85/"+string;
				if(parseInt(type)==1)
				{
					let object ={"patient_id":id,"branch_id":session_branch_id};
					let string = btoa(JSON.stringify(object));
					window.location.href = baseURL + "html_navigation/18/87/"+string;
				}
				else
				{
					let object ={"patient_id":id,"branch_id":session_branch_id};
