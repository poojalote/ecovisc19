...
$email = $this->session->user_session->user_name;
$user_type = 1;
 $group_id = $this->session->user_session->group_id;
...
<head>
	...
	<script>
		function init() { window.parent.setUpFrame(); console.log("done"); return true; }
		function SetName() {
			if (window.opener != null && !window.opener.closed) {
				window.parent.setUpFrame(1);
			}
		}
		window.addEventListener("beforeunload", function(e){
			if (window.opener != null && !window.opener.closed) {
				window.parent.setUpFrame(0);
			}
		}, false);
...
<script src="<?= base_url() ?>assets/modules/jquery-validation/js/jquery.validate.min.js"
