...
//            $this->MasterModel->sendSMS($mobile,array('otp'=>$LastOtp),'1107162869107284120','3');
            ...
            if ($checkOtpExists > 0) {
                $updateOtp = $this->MasterModel->_update('otp_master',
					array('otp' => $rnd_no, 'expiry_on' => $date,'created_on'=>date('Y-m-d H:i:s')), array('user_id' => $user_id, 'branch_id' => $branch_id, 'user_type' => $user_type));
            } else {
                $insertOtp = $this->MasterModel->_insert('otp_master',
					array('otp' => $rnd_no, 'user_id' => $user_id, 'branch_id' => $branch_id, 'user_type' => $user_type, 'expiry_on' => $date,'created_on'=>date('Y-m-d H:i:s')));
            }
